            ...
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if(in_array(Auth::user()->role, ['mahasiswa', 'dosen']))
                        <button class="btn btn-sm btn-success" onclick="modalActionLombaPublik('{{ route('lomba.user.create_form') }}', 'Ajukan Info Lomba Baru', 'modalFormLombaUserPublik')">
...
{{-- Modal untuk Form Pengajuan Lomba oleh Mahasiswa/Dosen dari halaman publik --}}
@if(in_array(Auth::user()->role, ['mahasiswa', 'dosen']))
<div class="modal fade" id="modalFormLombaUserPublik" tabindex="-1" aria-labelledby="modalFormLombaUserPublikLabel" aria-hidden="true">
...
@push('js')
<script>
    function modalActionLombaPublik(url, title = 'Form', modalId = 'modalDetailLombaPublik') {
    ...
    $(document).ready(function () {
        $('#dataLombaPublik').DataTable({
            ...
            ajax: "{{ route('lomba.publik.list') }}",
            columns: [
                { data: 'DT_RowIndex', name: 'DT_RowIndex', className: 'text-center', orderable: false, searchable: false },
                { data: 'nama_lomba', name: 'nama_lomba' },
                { data: 'penyelenggara', name: 'penyelenggara' },
                { data: 'tingkat', name: 'tingkat', className: 'text-center' },
                { data: 'periode_pendaftaran', name: 'pembukaan_pendaftaran' },
                { data: 'biaya_formatted', name: 'biaya', className: 'text-end' },
