        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    @foreach ($globalData['criteria'] as $criterion)
                                        <th>{{ ucwords(str_replace('_', ' ', $criterion)) }} (C{{$loop->iteration}})</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <p>
                        Normalisasi dilakukan untuk menyeragamkan skala nilai dari berbagai kriteria. Setiap nilai dalam matriks keputusan dibagi dengan pembagi (divisor) untuk kriterianya. Pembagi dihitung menggunakan rumus:
                        $$ \text{Divisor}_j = \sqrt{\sum_{i=1}^{m} x_{ij}^2} $$
                        Dimana $x_{ij}$ adalah nilai alternatif ke-$i$ pada kriteria ke-$j$, dan $m$ adalah jumlah total alternatif.
                    </p>
                    <strong>Nilai Pembagi:</strong>
                    <ul>
                        @foreach ($globalData['divisors'] as $criterion => $divisor)
                            <li><strong>{{ ucwords(str_replace('_', ' ', $criterion)) }}:</strong> {{ number_format($divisor, 4) }}</li>
                        @endforeach
                    </ul>
                    <p class="mt-3">
                        Matriks ternormalisasi ($r_{ij}$) dihitung dengan rumus: $r_{ij} = x_{ij} / \text{Divisor}_j$.
                    </p>
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                             <thead>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $result['lomba']->nama_lomba }}</td>
                                    @foreach ($globalData['criteria'] as $criterion)
                                        <td>{{ number_format($result['normalized_values'][$criterion] * $globalData['weights'][$criterion], 4) }}</td>
                                    @endforeach
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    <p>
                        Nilai akhir (skor optimasi) untuk setiap alternatif dihitung dengan menjumlahkan nilai terbobot dari kriteria <strong>Benefit</strong> dan menguranginya dengan jumlah nilai terbobot dari kriteria <strong>Cost</strong>.
                        $$ Y_i = \sum_{j=1}^{g} v_{ij} - \sum_{j=g+1}^{n} v_{ij} $$
                        Dimana $g$ adalah jumlah kriteria benefit. Alternatif kemudian diurutkan dari nilai $Y_i$ tertinggi ke terendah.
                    </p>
                     <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                             <thead>
                                ...
                                @foreach ($results as $result)
                                    @php
                                        ...
                                        foreach ($globalData['benefit_criteria'] as $c) {
                                            $weightedValue = $result['normalized_values'][$c] * $globalData['weights'][$c];
