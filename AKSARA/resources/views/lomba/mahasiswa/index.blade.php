                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="GET" id="filterFormAdminLomba" class="row g-3 mb-3 align-items-center">
                            <div class="col-md-4">
                                <input type="text" class="form-control form-control-sm" id="search_nama_admin"
                            ...
                            <div class="col-md-3">
                                <select class="form-select form-select-sm" id="filter_status_admin" name="filter_status">
                                    ...
                                    <option value="pending" {{ request('filter_status') == 'pending' ? 'selected' : '' }}>
                                        Pending</option>
                                    <option value="disetujui" {{ request('filter_status') == 'disetujui' ? 'selected' : '' }}>
                                        Disetujui</option>
                            ...
                            <div class="col-md-2">
                                <a href="{{ route('lomba.index') }}" class="btn btn-secondary btn-sm w-50">Reset</a>
                            </div>
                            <div class="col-md-2">
                                <button id="btnRekomendasi" class="btn btn-primary">Rekomendasi</button>
                            </div>
                        </form>
...
@push('js')
    <script>
        ...
        var isRekomendasiAktif = false; // Status tombol rekomendasi

        function modalAction(url, modalId = 'myModalAdmin') {
        ...
        $(document).ready(function () {
            ...
            $('#filterFormAdminLomba').on('submit', function (e) {
                e.preventDefault();
                isRekomendasiAktif = false; // reset ke default
                dataDaftarLombaAdmin.ajax.reload();
            });

            $('#btnRekomendasiAdmin').on('click', function () {
