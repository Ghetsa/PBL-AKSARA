...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
<style>
    /* Sedikit gaya untuk form filter */
    #filterFormLomba .btn {
        margin-top: 1.85rem;
    }
    .visually-hidden {
...
{{-- Modal Detail Perhitungan MOORA --}}
<div class="modal fade" id="detailMooraModal" tabindex="-1" aria-labelledby="detailMooraLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="contentDetailMoora">
                    <p class="text-center">Memuat detail perhitungan...</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    let dataDaftarLomba;
    let isRekomendasiAktif = false;

    // Fungsi memuat detail perhitungan MOORA ke modal
    function loadDetailMoora(lombaId) {
        const url = "{{ route('lomba.detailMoora') }}";
        $.ajax({
            url: url,
            type: 'GET',
            data: { lomba_id: lombaId },
            beforeSend: function() {
                $('#contentDetailMoora').html('<p class="text-center">Memuat detail perhitungan...</p>');
                new bootstrap.Modal(document.getElementById('detailMooraModal')).show();
            },
            success: function(response) {
                $('#contentDetailMoora').html(response);
            },
            error: function(xhr) {
                ...
                $('#contentDetailMoora').html('<div class="alert alert-danger">' + msg + '</div>');
            }
        });
    }
    ...
    $(document).ready(function() {
        dataDaftarLomba = $('#dataDaftarLomba').DataTable({
            ...
            ajax: {
                ...
                error: function(xhr, error, thrown) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal Memuat Data',
                        text: 'Tidak dapat mengambil data lomba. Silakan coba lagi nanti.',
                    });
                    $('#dataDaftarLomba_processing').hide();
                }
            },
            ...
            order: [],
            drawCallback: function(settings) {
                // Tampilkan/ sembunyikan info rekomendasi & ubah header kolom Skor
                if (isRekomendasiAktif) {
                    $('#infoRekomendasi').slideDown();
                    $('#thSkorMoora').text('Skor MOORA');
                } else {
                    $('#infoRekomendasi').slideUp();
                    $('#thSkorMoora').text('Skor');
                }
                ...
                // Pasang event click tombol detail hitungan
                $('.btn-detail-hitungan').off('click').on('click', function() {
                    const lombaId = $(this).data('id');
                    loadDetailMoora(lombaId);
                });
            }
        });
        ...
        $('#btnRekomendasi').on('click', function() {
            ...
            dataDaftarLomba.ajax.reload(function(json) {
                Swal.close();
                if (json && json.recordsFiltered > 0) {
                    Swal.fire({
                        icon: 'success',
                    ...
                    Swal.fire({
                        icon: 'info',
                        title: 'Tidak Ada Rekomendasi',
                        text: 'Belum ada lomba yang sesuai dengan preferensi Anda.',
                    });
                }
            }, false);
        });
