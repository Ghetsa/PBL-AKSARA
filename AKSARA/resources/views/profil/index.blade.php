                        ...
                        <div class="tab-pane show active" id="profile-overview" role="tabpanel" aria-labelledby="profile-tab-1">
                            <div class="row">
                                    ...
                                    <div class="card">
                                        <div class="card-body position-relative">
                                            @php
                                                ...
                                                // Ambil path foto profil jika ada dan valid
                                                ...
                                                <div class="mb-2 pb-2 @if(!$loop->last) border-bottom @endif">
                                                    ...
                                                    @if($k->sertifikasi && Storage::disk('public')->exists($k->sertifikasi))
                                                        <small class="text-muted d-block">
                                                            Sertifikasi: <a href="{{ Storage::url($k->sertifikasi) }}" target="_blank" class="btn btn-link btn-sm p-0">Lihat File</a>
                                                        </small>
                                                    @elseif($k->sertifikasi)
                                                        <small class="text-info d-block">Sertifikasi: {{ $k->sertifikasi }} (Catatan)</small>
                                                    @else
                                ...
                                <div class="col-lg-8 col-xxl-9">
                                        ...
                                        @if($user->minat && $user->minat->count() > 0)
                                            <div class="card-body">
                                                @foreach($user->minat as $m)
                                                    <span class="badge bg-light text-dark me-1 mb-1 p-2 fs-6">{{ $m->nama_minat ?? $m->minat }}</span>
                                                @endforeach
                                            </div>
                                        @else
                                            <div class="card-body">
                                    ...
                                    @if($user->role == 'mahasiswa' || $user->role == 'dosen') {{-- Sesuaikan jika dosen juga menampilkan ini --}}
                                    <div class="card mt-3">
                                        <div class="card-header d-flex justify-content-between align-items-center">
                                            <h4>Pengalaman</h4>
                                        ...
                                        @if($user->pengalaman && $user->pengalaman->count() > 0)
                                            <div class="card-body">
                                                <ul class="list-group list-group-flush">
                                                    ...
                                                    <li class="list-group-item px-0 @if(!$loop->first) pt-3 @else pt-0 @endif @if(!$loop->last) pb-3 @else pb-0 @endif">
                                        ...
                                        <div class="card-body">
                                            <ul class="list-group list-group-flush">
                                                @foreach($user->mahasiswa->prestasi as $index => $pres)
                                                    @if($index < 3) {{-- Tampilkan maksimal 3 prestasi di profil, sisanya di halaman prestasi --}}
                                                    <li class="list-group-item px-0 @if(!$loop->first) pt-3 @else pt-0 @endif @if(!$loop->last) pb-3 @else pb-0 @endif">
                                                        <p class="mb-0 text-lg">{{ $pres->nama_prestasi }}
                                                            <span class="text-muted">({{ $pres->tingkat }} - {{ $pres->tahun }})</span>
                                                        </p>
                                                        @if($pres->file_bukti && Storage::disk('public')->exists($pres->file_bukti))
                                                          <a href="{{ Storage::url($pres->file_bukti) }}" target="_blank" class="btn-link text-sm">Lihat Bukti</a>
                                                        @endif
                                                    </li>
                                                    @endif
                                                @endforeach
                                                @if($user->mahasiswa->prestasi->count() > 3)
                                                    <li class="list-group-item px-0 pt-3 pb-0 text-center">
                                                        <a href="{{ route('prestasi.mahasiswa.index') }}">Lihat semua prestasi...</a> {{-- Asumsi route ke halaman list prestasi mahasiswa --}}
                                                    </li>
                                                @endif
                                            </ul>
                                        </div>
...
<style>
    .highlight-section {
        ...
        border-left: 3px solid #0d6efd; /* Contoh highlight dengan border biru */
        padding-left: 10px; /* Beri sedikit padding agar border terlihat */
    }
    @keyframes highlightAnimation {
        0% { background-color: rgba(13, 110, 253, 0.1); } /* Biru muda transparan */
        70% { background-color: rgba(13, 110, 253, 0.1); }
        100% { background-color: transparent; }
    }
    .wid-auto { /* Untuk avatar agar tidak pecah jika ukuran asli kecil */
        max-width: 100px; /* Sesuaikan dengan ukuran yang Anda inginkan */
        height: auto;
    }
    .text-lg { /* Ukuran font sedikit lebih besar untuk item list */
        font-size: 1.05rem; /* Sesuaikan */
    }
</style>
@endpush
{{-- @extends('layouts.template')
                                               ...
                                               <ul class="list-group list-group-flush">
                                                    ...
                                                    <li class="list-group-item px-0 pt-0">
                                                        <p class="mb-1 text-lg" text-lg>{{ $p->pengalaman_nama }}
                                                            <span class="text-muted text-lg">({{ $p->pengalaman_kategori }})</span>
                                                        </p>
                                                    </li>
                                                    @endforeach
                                                </ul>
                                                ...
                                                <ul class="list-group list-group-flush">
                                                    ...
                                                    <li class="list-group-item px-0 pt-0">
                                                        <p class="mb-1">{{ $p->pengalaman_nama }}
                                                            <span class="text-muted">({{ $p->pengalaman_pengalaman_kategori }})</span>
                                                        </p>
                                                    </li>
                                                    @endforeach
                                                </ul>
