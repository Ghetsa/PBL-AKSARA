...
<div class="container-fluid">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <form action="{{ route('dosen.notifikasi.markAllAsRead') }}" method="POST">
                @csrf
                                ...
                                <div class="d-flex align-items-start">
                                    ...
                                    <div class="ms-3 notification-actions d-flex flex-nowrap">
                                        ...
                                        <button class="btn btn-sm btn-primary" 
                                                onclick="showNotificationDetail('{{ route('dosen.notifikasi.show_and_read', ['id' => $notif->id, 'model' => $notif->type]) }}')">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        
                                        @if($notif->status_baca == 'belum_dibaca')
                                            <form action="{{ route('dosen.notifikasi.mark_as_read', ['id' => $notif->id, 'model' => $notif->type]) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-outline-success" title="Tandai Dibaca"><i class="fas fa-check"></i></button>
                                            </form>
                                        @endif
                                        
                                        <form action="{{ route('dosen.notifikasi.destroy', ['id' => $notif->id, 'model' => $notif->type]) }}" method="POST" class="form-hapus">
                                            @csrf
                                            @method('DELETE')
...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        @if ($unreadCount > 0)
            <form action="{{ route('dosen.notifikasi.markAllAsRead') }}" method="POST" onsubmit="return confirm('Anda yakin ingin menandai semua notifikasi sebagai telah dibaca?');">
                @csrf
    ...
    <div class="card">
        <div class="card-header pb-0">
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <a class="nav-link @if($filter === 'semua') active @endif" href="{{ route('dosen.notifikasi.index', ['filter' => 'semua']) }}">Semua</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link @if($filter === 'belum_dibaca') active @endif" href="{{ route('dosen.notifikasi.index', ['filter' => 'belum_dibaca']) }}">Belum Dibaca</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link @if($filter === 'sudah_dibaca') active @endif" href="{{ route('dosen.notifikasi.index', ['filter' => 'sudah_dibaca']) }}">Sudah Dibaca</a>
                </li>
            </ul>
        </div>
                    ...
                    @foreach($allNotifications as $notif)
                        <div class="notification-item d-flex justify-content-between align-items-center @if($notif->status_baca == 'belum_dibaca') unread @endif">
                            ...
                            <div class="actions d-flex">
                                <a href="{{ route('dosen.notifikasi.show_and_read', ['id' => $notif->id, 'model' => $notif->type]) }}" class="btn btn-sm btn-outline-primary">Detail</a>
                                
                                @if($notif->status_baca == 'belum_dibaca')
                                <a href="{{ route('dosen.notifikasi.show_and_read', ['id' => $notif->id, 'model' => $notif->type]) }}" class="btn btn-sm btn-outline-success" title="Tandai Dibaca">Tandai Dibaca</a>
                                @endif
