...
class PrestasiController extends Controller
{
    ...
    public function listDosen(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!$dosen) {
                return response()->json(['error' => 'Profil dosen tidak ditemukan.'], 403);
            }
             $data = PrestasiModel::with('mahasiswa.user') // << Tambahkan eager loading
                ->where('dosen_id', $dosen->dosen_id)
                ->select(['prestasi_id', 'mahasiswa_id', 'nama_prestasi', 'kategori', 'tingkat', 'tahun', 'status_verifikasi', 'file_bukti', 'dosen_id']) // pastikan dosen_id disertakan
                ->orderBy('tahun', 'desc');
